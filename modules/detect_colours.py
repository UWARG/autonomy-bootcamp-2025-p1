...
class DetectRed:
    ...
    def run(self, image: str, output_path: Path, return_mask: bool = False) -> None | np.ndarray:
        ...
        lower_red_two = np.array([150, 30, 30])
        upper_red_two = np.array([179, 255, 255])

        # Apply the threshold for the colour detection
        mask_one = cv2.inRange(hsv, lower_red, upper_red)
        mask_two = cv2.inRange(hsv, lower_red_two, upper_red_two)

        mask_final = mask_one | mask_two

        # Annotate the colour detections
        # replace the '_' parameter with the appropiate variable
        contours, _ = cv2.findContours(mask_final, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
        # ============
        # ↑ BOOTCAMPERS MODIFY ABOVE THIS COMMENT ↑
        # ============
        ...
        return mask_final if return_mask else None
