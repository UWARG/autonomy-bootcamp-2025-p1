...
class DetectRed:
    ...
    def run(self, image: str, output_path: Path, return_mask: bool = False) -> None | np.ndarray:
        ...
        # Set upper and lower bounds for colour detection, this is in HSV
        lower_red1 = np.array([0, 80, 10])
        upper_red1 = np.array([10, 255, 255])
        lower_red2 = np.array([170, 80, 10])
        upper_red2 = np.array([180, 255, 255])

        # Apply the threshold for the colour detection
        mask_orange_side = cv2.inRange(hsv, lower_red1, upper_red1)
        mask_magenta_side = cv2.inRange(hsv, lower_red2, upper_red2)
        mask = cv2.bitwise_or(mask_orange_side, mask_magenta_side)
