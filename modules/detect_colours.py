...
class DetectBlue:
    ...
    def run(self, image: str, output_path: Path, return_mask: bool = False) -> None | np.ndarray:
        ...
        if img is None:
            # logger.warn('Image could not be opened')
            return None
        ...
        upper_blue = np.array([110, 240, 255])
...
class DetectRed:
    ...
    def run(self, image: str, output_path: Path, return_mask: bool = False) -> None | np.ndarray:
        ...
        # Set upper and lower bounds for colour detection, this is in HSV
        # Since red takes [0-30] and [120-179] we will use two masks
        lower_red_lo = np.array([0, 30, 0])
        upper_red_lo = np.array([30, 255, 190])
        lower_red_hi = np.array([160, 30, 0])
        upper_red_hi = np.array([179, 255, 190])

        # Apply the threshold for the colour detection
        mask_lo = cv2.inRange(src=hsv, lowerb=lower_red_lo, upperb=upper_red_lo)
        mask_hi = cv2.inRange(src=hsv, lowerb=lower_red_hi, upperb=upper_red_hi)
