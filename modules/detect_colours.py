...
class DetectBlue:
    ...
    def run(self, image: str, output_path: Path, return_mask: bool = False) -> None | np.ndarray:
        ...
        lower_blue = np.array([90, 100, 40])  # 90 degrees, 100 sa  turation, 40 brightness
        upper_blue = np.array([135, 255, 255])  # 135, maximum values for saturation and brightness
...
class DetectRed:
    ...
    def run(self, image: str, output_path: Path, return_mask: bool = False) -> None | np.ndarray:
        ...
        return full_mask if return_mask else None
