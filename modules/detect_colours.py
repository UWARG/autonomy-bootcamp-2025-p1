...
class DetectRed:
    ...
    def run(self, image: str, output_path: Path, return_mask: bool = False) -> None | np.ndarray:
        ...
        upper_lower_red = np.array([170, 100, 100])
        upper_upper_red = np.array([180, 255, 255])

        # Apply the threshold for the colour detection
        lower_mask = cv2.inRange(hsv, lower_lower_red, lower_upper_red)
        upper_mask = cv2.inRange(hsv, upper_lower_red, upper_upper_red)
